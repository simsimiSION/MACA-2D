...
class SelfPlaySimulator():
    ...
    def step(self, actions, enemy_actions):
        # update detect info
        ...
        ally_be_attack_list, enemy_attack_val = self._convert_action_to_be_attacked_list(self.enemies, self.allies, enemy_actions)
        
        # ally take action
        for ally, action, ally_be_attack in zip(self.allies, actions, ally_be_attack_list):
            direct = action[0]
            attack = action[1]
            ally.step(direct, ally_be_attack, attack)
        ...
        return self.get_game_status(ally_attack_val, enemy_attack_val)
    
    def get_game_status(self, ally_attack_val, enemy_attack_val):
        game_status = {}
        ...
        for i, enemy in enumerate(self.enemies):
            ...
            if enemy_attack_val:
                game_status['enemy_info'][enemy.id]['damage_val'] = enemy_attack_val[i]
