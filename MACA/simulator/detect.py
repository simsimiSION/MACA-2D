...
class DetectSimulator():
    ...
    def initialize(self, random_side=False):
        ...
        for i, enemy in enumerate(self.enemies):
            ...
            # pos initialize
            if enemy.type == FIGHTER_TYPE['reconnaissance']:
                pos = np.array([enemies_reconnaissance_x_bias, self.enemies_reconnaissance_y_resolution*(i+1)]) + np.random.uniform(-1, 1, size=(2)) * self.args.simulator.random_limit
            else:
    ...
    def step(self, actions):
        ...
        for enemy, action, enemy_be_attack in zip(self.enemies, enemy_actions, enemy_be_attack_list):
            ...
            direct = np.clip(np.random.randn(1), -0.52, 0.52)#action[0]
            attack = action[1]
