...
class ReconnDetectEnv(MultiAgentEnv):
    def __init__(self, config):
        ...
        # set simulator
        self.simulator = DetectSimulator(args, self.allies, self.enemies)
        self.n_ally = self.simulator.n_ally
        self.n_enemy = self.simulator.n_enemy

        # set env_wrapper
        self.env_wrapper = ReconnDetectAdjWrapper(args, self.simulator)
        ...
        # RL wrapper
        self.observation_space = Box(low=-999.9, high=999.9, shape=(4+6+(self.n_ally+self.n_enemy-1)*6+self.n_ally*self.n_ally,))
